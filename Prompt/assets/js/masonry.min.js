class masonry {
    constructor({
      ...
      links         = document.querySelectorAll('[data-masonry-link]'),
      active        = 'active',
      margin        = 20,
      responsive    = {
        980: {
    ...
    handleFilterClick(ev, element){
      ...
      if(element.dataset.masonryLink === this.dataLink){
        return
      } else {
        this.dataLink = element.dataset.masonryLink
        links.forEach(el => {
    ...
    init(){
      ...
      links.forEach((el, id) => {
        if(id === 0){
          ...
          this.dataLink = el.dataset.masonryLink
        }
    ...
    _filterElements(callback){
      ...
      this.activeElements = elements.filter(el => {
        if(dataLink === 'all') {
          ...
          if(el.dataset.masonryEl !== dataLink) {
